...
def test_init_item_assets_success(brandos_service):
    folder_id = "2026-01-01-semana-fake1"
    item_id = "post-segunda"
    
    manifest_path = brandos_service.init_item_assets(folder_id, item_id)
...
def test_init_item_assets_error(brandos_service):
    with pytest.raises(ValueError, match="Peça não encontrada"):
        brandos_service.init_item_assets("2026-01-01-semana-fake1", "item-nao-existe")

def test_upload_item_asset_success(brandos_service):
    folder_id = "2026-01-01-semana-fake1"
    item_id = "post-segunda"
    brandos_service.init_item_assets(folder_id, item_id)
    
    file_content = b"fake image content"
    file_name = "test_image.jpg"
    
    result_path = brandos_service.upload_item_asset(
        folder_id=folder_id,
        item_id=item_id,
        file_name=file_name,
    ...
    manifest_path = os.path.join(brandos_service.assets_dir, f"{folder_id}-{item_id}", "manifest.json")
    with open(manifest_path, "r", encoding="utf-8") as f:
        manifest = json.load(f)
        
    assert any(img["file_name"] == "test_image.jpg" for img in manifest["files"]["images"])

def test_upload_item_asset_error(brandos_service):
    ...
    with pytest.raises(ValueError, match="Pasta de assets não inicializada"):
        brandos_service.upload_item_asset(
            folder_id=folder_id,
            item_id=item_id,
            file_name="test.jpg",
            file_content=b"123",
...
def test_add_item_asset_prompt_success(brandos_service):
    folder_id = "2026-01-01-semana-fake1"
    item_id = "post-segunda"
    brandos_service.init_item_assets(folder_id, item_id)
    
    brandos_service.add_item_asset_prompt(folder_id, item_id, "Draw a cool cat")
...
def test_import_recommended_prompt(brandos_service):
    ...
    brandos_service.import_recommended_prompt(folder_id, item_id, "07-prompts.md")
...
def test_delete_item_asset(brandos_service):
    folder_id = "2026-01-01-semana-fake1"
    item_id = "post-segunda"
    brandos_service.init_item_assets(folder_id, item_id)
    ...
    # Delete it
    brandos_service.delete_item_asset(folder_id, item_id, "pdf", "todelete.pdf")
    
    with open(manifest_path, "r", encoding="utf-8") as f:
        manifest = json.load(f)
    assert len(manifest["files"]["pdf"]) == 0
    
    asset_file_path = os.path.join(brandos_service.assets_dir, f"{folder_id}-{item_id}", "pdf", "todelete.pdf")
    assert not os.path.exists(asset_file_path)

def test_delete_item_prompt(brandos_service):
    folder_id = "2026-01-01-semana-fake1"
    item_id = "post-segunda"
    brandos_service.init_item_assets(folder_id, item_id)
    
    brandos_service.add_item_asset_prompt(folder_id, item_id, "Draw a dog")
    ...
    # Delete by id
    brandos_service.delete_item_prompt(folder_id, item_id, prompt_id=prompt_id)
